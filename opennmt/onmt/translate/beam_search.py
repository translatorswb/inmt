...
class BeamSearch(DecodeStrategy):
    ...
    def advance(self, log_probs, attn, partial=None, partialf=None):
        ...
        if partialf and step and step == len(partial) + 1:	
            ...
            #     proxyf.extend(partialf)	
            # proxyf = torch.tensor(proxyf)	
            # proxyf = proxyf.view(1, self.beam_size * vocab_size)	
            # curr_scores = torch.mul(curr_scores.view(self.beam_size * vocab_size), proxyf)	
    
            # This is the optimized code, however it is in interest to optimize the above code	
            proxyf = []	
            for i in range(self.beam_size):	
                proxyf.extend([k+(i*vocab_size) for k in partialf])	
            indices = torch.tensor(proxyf)	
            mask_array = torch.zeros(self.beam_size * vocab_size)	
            mask_array.fill_(float('inf'))	
            mask_array[indices] = 1	
            mask_array = mask_array.view(1, self.beam_size * vocab_size)	
            curr_scores = torch.mul(curr_scores.view(self.beam_size * vocab_size), mask_array)	
            # curr_scores = curr_scores.view(1, self.beam_size * vocab_size)
